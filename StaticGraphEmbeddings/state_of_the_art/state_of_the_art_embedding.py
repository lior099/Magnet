...
import os
from StaticGraphEmbeddings.state_of_the_art.gcn.utils import *
from StaticGraphEmbeddings.state_of_the_art.gcn.models import GCN
...
class GCNModel(StaticGraphEmbedding):
    """
    GCN embedding method
    """
    def __init__(self, params, method_name, graph, file_tags):
        ...
        self._adj, self._features, self._labels, self._idx_train, self._idx_val, self._idx_test = \
            load_data(graph, file_tags, len(graph.nodes()))
        self._seed = 42
...
def final(G, method_name, params, file_tags=None):
    ...
    if method_name == "HOPE":
        ...
        t = time()
        embedding = GCNModel(params, method_name, G, file_tags)
